...
\chapter{計算のテクニック}
  漸化式の解説を行う前に,実際の計算において重要なテクニックをこの章では説明する.
  ...
  \section{無理やり合わせる}
    ...
    \subsection{$\displaystyle \frac{a}{a}$を掛ける}
      この式変形は分母の有理化や数I\hspace{-1.2pt}I\hspace{-1.2pt}Iの積分や極限の問題でよく見られるものである.
      実例を上げる.
      \[
...
\chapter{漸化式の解説}
  \section{漸化式の分類}
    ...
    その定義は高校数学程度の範囲内で既知の関数に帰着するか,有限回の演算または操作により一般項を得られるような解法が存在するものを意味している.
  ...
  \section{基本4パターン}
        ...
        \item $\displaystyle a_1=0,\,a_{n+1}=a_n+\frac{1}{n(n+1)}$\\
              ...
              \begin{align*}
                a_n
                  ...
                  &+\cdots+\left(\frac{1}{n-1}-\frac1n\right)\\
                  &=1-\frac1n
              \end{align*}
    ...
    \subsection{階比型 \, $a_{n+1}=f(n)a_n$}
      ...
      式全体がごちゃごちゃしている場合と,両辺が同じような見た目をしている場合を比べると,後者のほうが断然法則性が見えやすいのは説明するまでもない.
      ...
      \begin{enumerate}[label=(\arabic*),parsep=3pt]
        ...
        \item $\displaystyle a_1=2,\, a_{n+1}=\frac{n+2}{n}a_n$
              ...
              \begin{align*}
                ...
                &\frac{a_{n-2}}{(n-2)(n-1)}=\frac{a_{1}}{1(1+1)}\\
...
\section{応用7パターン}
  ...
  \subsection{階差等比複合型 \, $\displaystyle a_{n+1}=pa_n+f(n)$}
    \subsubsection{一般型 \, $\displaystyle a_{n+1}=pa_n+f(n)$}
      ...
      この変形の根拠は,特性方程式型の解法と似ている.
      $f(n)$と次数の等しい$g(n)$によって
      \[
        g(n+1)=pg(n)+f(n)
      \]
      というような式を立てれば,
      \[
        ...
        \end{array}
      \]
      のようにして,特性方程式型のような解き方で解を得ることも可能である.
    ...
    \subsubsection{1次分数型 \, $\displaystyle a_{n+1}=\frac{pa_n+q}{ra_n+s}$}
      ...
      \begin{enumerate}[label=(\arabic*),parsep=3pt]
        ...
        \item $\displaystyle a_1=1,\, a_{n+1}=1+\frac{1}{a_n}$\\
              ...
              \end{align*}        
      \end{enumerate}
    余談にはなるが,最後の問題はフィボナッチ数列の2項間の比の極限値は黄金比$\frac{1\sqrt{5}}{2}$に一致する.
    また,ここから派生したリュカ数列やトリボナッチ数列という数列も存在する.
    これらは黄金比を中心として様々な興味深い性質を持つが,本書の趣旨から外れてしまうため割愛する.
  ...
  また,階乗で割ったり,対数を使うことでうまく解けるかも同時に検討する.
  ...
  \[
    a_{n+1}=\frac{pa_n+q}{ra_n+s}
  \]
  の場合では,まず不動点を探す.不動点の方程式が逆数のみで一次式になる場合は
  $\displaystyle b_n=\frac{1}{a_n}$ を使う.不動点が二つある,つまり不動点の方程式が2次の場合は,
  \[
    b_n=\frac{a_n-\alpha}{a_n-\beta}
  \]
  ...
  \begin{description}
    \item[等差型]
      ...
      $n$ の式なら階差を足し戻す.
    \item[等比型]
      $\displaystyle \frac{a_{n+1}}{a_n}$ を計算する.一定なら等比型,
      $n$ の式なら比を掛け合わせる.
    \item[一次式型]
    ...
    \item[階差型]
      ...
      へ直す.
    \item[階比型]
      $\displaystyle \frac{a_{n+1}}{a_n}=g(n)$ なら,
      \[
        a_n=a_1\prod_{k=1}^{n-1}g(k)
      \]
      へ直す.
